...
A German trainer that runs offline in any browser. It holds three independent **courses**, picked with a toggle above the nav tabs: *verb forms* (270 irregular verbs — Präsens/Präteritum/Perfekt), *verbs with prepositions* (111 fixed `Verb + Präposition + Kasus` combinations) and *Lernwortschatz B2 Beruf* (1135 words and phrases from the 14 chapters of *Linie 1 Beruf B2*). All three share every screen (trainer, dictionary, cards) but keep separate ranges and separate progress. There is no build system, package manager, linter, or test framework — the files are served to the browser exactly as they sit on disk. UI languages are English (default), Russian, Italian, and Turkish; repo docs and code comments are in Russian.
...
| `js/choices.js` | multiple-choice distractors: `buildChoices()`, `buildTransChoices()`, `transDistractors()`, `prepDistractors()`, `perfDistractors()`, `wortDistractors()` |
...
The «Как отвечать» radio pair on the trainer screen (`input[name="ansmode"]`, read once per test by `selectedAnsMode()` and stored on `session.ansMode`) decides how **both** fields are answered: in choice mode the translation gets its own four options and the second field gets four of its own. `transChoiceOn()` is `session.ansMode === "choice" && session.askT`, `choiceOn()` is the same with `askSecond()`; when both are false the screen behaves exactly as with typed input. Nothing about the mode is persisted, but each course carries a default in `COURSES[...].test` (`{trans, perf, ansMode}`) that `applyTestDefaults()` writes into the checkboxes on start and on every course switch — the Lernwortschatz course starts as translation only + choice (article/plural off, since most of its entries are not nouns), the other two as second-field-only + choice.

Whichever field is *not* being asked is shown as a given: `renderGiven()` fills `#qgiven` under the headword with the translation when `session.askT` is false and with `secondOf(v)` when the second field is off (skipped when the word has no second answer at all, so a Lernwortschatz verb shows nothing there). Its captions are `dheadTrans` / `tc("dheadPerfekt")`, the same short labels the dictionary headers use — no new label keys. With both fields on the line stays hidden.

In choice mode `renderQuestion()` hides `#inTrans`/`#inPerf` (and the Kasus/article chips) and fills `#transChoices` and `#choiceList` with four `.choice` buttons each, via `renderChoiceBox(box, data, offset)`. The badge numbers are the keyboard keys: the translation list is always 1–4, the second list is 5–8 when translation options are on screen and 1–4 when they are not (`choiceOffset()`), and `pickByKey()` in `js/quiz.js` routes a digit to the right list — the handler in `js/main.js` ignores digits typed inside an `<input>`. `doCheck()` bails out with `alertPickOption` if *either* visible list has nothing selected — before any grading, so the question stays re-answerable — then compares the picks with `session.tchoices.correct` / `session.choices.correct` and paints each list via `markChoices(box, correct, pick)`. The «Засчитать перевод?» override chips only appear in input mode: a picked option is either the right string or not. `curEval.ansT`/`curEval.ansP` hold the picked strings, so the mistake table needs no special case.

`js/choices.js` builds the four options; `buildTransChoices(v)` (translation) and `buildChoices(v)` (second field) both return `{correct, options}` with `options` already shuffled.

- **Translation** (all three courses): the wrong options are other words' translations, drawn first from the selected range — same chapter or same block, so the answer can't be picked out by topic — then from the whole course if the range is too small. Candidates are deduplicated through `normRu()`, so a word whose translation coincides with the right one never shows up as a distractor.
...
`t(key)` looks up the current language and falls back to `DEFAULT_LANG` (`en` — the language a first-time visitor gets; the toggle's button order is still registration order, so Russian stays the leftmost button) for a missing key, so a partially translated new language still renders. Static markup is translated via `data-i18n` / `data-i18n-placeholder` attributes resolved by `applyStaticI18n()`. Dynamic screens re-render on language switch inside `setLang()` — add new screens there if they show translated text.
...
Adding a *string* still means adding it to every `labels_*.js` — the fallback keeps the app working, but it shows English.
