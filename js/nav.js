...
function show(name){ Object.values(SCREENS).forEach(id=>document.getElementById(id).classList.add("hide")); document.getElementById(SCREENS[name]).classList.remove("hide");
  const navVisible = (name==="home"||name==="dict"||name==="learnSetup");
  document.getElementById("nav").classList.toggle("hide", !navVisible);
  document.getElementById("courseToggle").classList.toggle("hide", !navVisible);
