...
document.addEventListener("keydown",e=>{ if(document.getElementById("quizScreen").classList.contains("hide"))return;
  ...
  /* 1–4 — вариант перевода, 5–8 — второе поле; в поле ввода цифры остаются цифрами */
  else if(/^[1-8]$/.test(e.key) && e.target.tagName!=="INPUT"){ e.preventDefault(); pickByKey(+e.key); } });
